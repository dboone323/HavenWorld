...
curl -s -m 20 -X POST http://127.0.0.1:3000/api/auth/register -H 'Content-Type: application/json' \
  -d "{\"username\":\"$U\",\"email\":\"$U@example.com\",\"password\":\"$P\"}" -o /tmp/reg.json -w "http=%{http_code}\n"
...
curl -s -m 20 -X POST http://127.0.0.1:3000/api/auth/login -H 'Content-Type: application/json' \
  -d "{\"username\":\"$U\",\"password\":\"$P\"}" -c /tmp/cj.txt -o /tmp/login.json -w "http=%{http_code}\n"
python3 -c "
import json
...
" 2>/dev/null || head -c 400 /tmp/login.json
echo
...
TOKEN=$(python3 -c "import json;print(json.load(open('/tmp/login.json')).get('accessToken',''))" 2>/dev/null)
curl -s -m 15 http://127.0.0.1:3000/api/users/me -H "Authorization: Bearer $TOKEN" -o /tmp/me.json -w "GET /api/users/me = %{http_code}\n"
head -c 300 /tmp/me.json; echo
...
curl -s -m 15 -X POST http://127.0.0.1:3000/api/auth/refresh -b /tmp/cj.txt -c /tmp/cj.txt -o /tmp/refresh.json -w "refresh=%{http_code}\n"
head -c 200 /tmp/refresh.json; echo
