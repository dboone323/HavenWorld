...
probe_register() { # $1 port
  ...
  curl -s -o /tmp/np.json -w '  status=%{http_code} total_time=%{time_total}\n' --max-time 20 \
  ...
  head -c 160 /tmp/np.json 2>/dev/null; echo
}

echo "=== PHASE A: exactly what Playwright starts (pnpm --filter server dev = nodemon) ==="
env $COMMON_ENV PORT=3000 nohup pnpm --filter server dev > /tmp/np_nodemon.log 2>&1 &
if wait_health 3000; then echo "  health OK on 3000"; else echo "  health FAILED"; tail -15 /tmp/np_nodemon.log; fi
probe_register 3000
echo "  --- nodemon log tail ---"
tail -12 /tmp/np_nodemon.log
...
env NODE_ENV=test SERVER_AUTOSTART=true ALPHA_INVITE_ONLY=false PORT=3225 DATABASE_URL="$TEST_URL" \
  REDIS_URL="redis://127.0.0.1:6379/1" nohup pnpm exec tsx src/index.ts > /tmp/np_tsx.log 2>&1 &
if wait_health 3225; then echo "  health OK on 3225"; else echo "  health FAILED"; tail -15 /tmp/np_tsx.log; fi
probe_register 3225
