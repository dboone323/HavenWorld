...
- [x] **A.2 Pure 2D Chibi Billboard Rendering**: Ensure `ChibiBillboard` plane is the sole visible representation of local and remote players with 8-frame walk, 2-frame idle, and direct sitting sprite cycles with `MATERIAL_ALPHATESTANDBLEND` transparency.
- [x] **A.3 Production Multi-Directional MiPlanet 2D Chibi Underwear Character & Layered Wearables**: Built with authentic 1774x887 18-frame sprite grid extracted directly from MiPlanet (`https://miplanet-online.onrender.com`):
  ...
  - Multi-sided angles: Front (idle, walk left 1-3, walk right 1-3), Back (idle, walk left 1-3, walk right 1-3), and Sitting (front sit left/right, back sit left/right).
  - Layered wearable components: Tops (`pink-sweater`), Bottoms (`olive-shorts`, `denim-jeans`), Eyes (`blue-eyes`, `brown-eyes`), Headwear (`trapper-hat`), Face Wear (`skull-balaclava`), Backwear (`wings-black`, `wings-white`), Shoes (`sneakers-purple`), and Hair (`wavy-hair`, `bald`).
  - Compiled into high-resolution 3840x2304 RGBA atlas (96x128 per frame cell) via `scripts/build_miplanet_atlas.py`.
- [x] **A.4 MiPlanet-Standard Wardrobe & Style Catalog UI**: In `AvatarCustomizer.ts`, redesigned customizer modal to match MiPlanet's `.catalog-layout` with a left category rail (All, Tops, Pants, Eyes, Headwear, Face Wear, Backwear, Shoes, Hair), card item grid with equip badges, right-side 170x290 live layered preview stage (`#wardrobe-avatar`), angle switchers (Front, Back, Sit), "Strip to Underwear" reset action, and backend persistence (`/api/users/me/avatar`). Verified in live production.
