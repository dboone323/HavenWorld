/**
 * HavenWorld — Shared Catalog & Rotating Stock Engine
 *
 * Centralized catalog of furniture, clothing, domestic pets, and exclusive collectibles.
 * Features 5 standardized rarity tiers, dual currency support (coins & gems),
 * footprint dimensions for 4-way rotation, and deterministic 48-hour stock rotation.
 */

export type ItemRarity = 'common' | 'uncommon' | 'rare' | 'epic' | 'legendary';
export type ItemCurrency = 'coins' | 'gems';
...
export interface CatalogItemDef {
  ...
  category: 'furniture' | 'clothing' | 'pet';
  rarity: ItemRarity;
  icon: string;
  w?: number;
  h?: number;
  ...
  salvage?: { scrap_metal: number; timber: number };
  description?: string;
}

export const CATALOG_ITEMS: Record<string, CatalogItemDef> = {
  // --- Core Furniture ---
  'sofa': {
  ...
  'pet_cat': {
    ...
    w: 1, h: 1, isRotatable: true,
    salvage: { scrap_metal: 0, timber: 0 },
...
// --- Limited-Time Rotating Stock Showcase Pool ---
export const ROTATING_FEATURED_ITEMS: CatalogItemDef[] = [
  {
  ...
  },
  {
    ...
    category: 'furniture', rarity: 'legendary', icon: '🔮',
    w: 1, h: 1, isRotatable: true,
...
export const ROTATION_PERIOD_MS = 48 * 60 * 60 * 1000; // 48 hours

/**
 * Deterministically compute the rotating stock window epoch.
 */
export function getRotationWindow(timestampMs: number = Date.now()): number {
  return Math.floor(timestampMs / ROTATION_PERIOD_MS);
}

/**
 * Get the time remaining in ms until the rotating catalog changes.
 */
export function getTimeUntilNextRotation(timestampMs: number = Date.now()): number {
...
/**
 * Deterministically select 3 featured showcase items for the current 48-hour period.
 */
export function getRotatingFeaturedStock(timestampMs: number = Date.now()): CatalogItemDef[] {
  const windowIdx = getRotationWindow(timestampMs);
  const pool = ROTATING_FEATURED_ITEMS;
  const count = 3;
  const selected: CatalogItemDef[] = [];
